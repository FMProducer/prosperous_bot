...
class TopKCheckpointManager:
    ...
    def register_checkpoint(self, ep, metrics, pth_path, json_path=None):
        ...
        val = metrics.get(self.metric_key, -float('inf'))
...
def compute_norm_stats(npz_path: str, cfg: MasterConfig, norm_stats_path: Optional[str] = None) -> dict:
    ...
    for asset, keys in tqdm(asset_keys.items(), desc="Computing stats per asset"):
        ...
        try:
            ...
            # Filter channels based on config if specified
            if hasattr(cfg, 'data') and hasattr(cfg.data, 'datachannels'):
                target_channels = len(cfg.data.datachannels)
                if asset_data.shape[2] > target_channels:
...
def run_training_session(
    ...
    if agent_mode is None and cfg_mod is not None:
        agent_mode = getattr(cfg_mod, "AGENT_MODE", None)
    ...
    env_kwargs = {
        ...
        "mirror_mode": getattr(cfg, "mirror_mode", True), # Передаем из конфига
    }

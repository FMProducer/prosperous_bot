...
def apply_rolling_norm(obs: np.ndarray) -> np.ndarray:
    """
    Applies rolling Z-score normalization to the observation.
    obs shape: (Channels, Length, 1)
    Normalization is performed along the Length dimension (axis 1).
    """
    # obs: (C, L, 1)
    means = np.mean(obs, axis=1, keepdims=True)
    stds = np.std(obs, axis=1, keepdims=True)
...
def load_and_prep_data(npz_path: str, split_name: str, norm_stats: dict, cfg: MasterConfig = None) -> tuple[list, list]:
    ...
    for key in data_keys:
        ...
        # MODIFIED: Skip global normalization for Rolling Z-Score mode.
...
def evaluate_agent(
    ...
    for i in range(num_eval_episodes):
        ...
        obs = apply_rolling_norm(obs) # Apply rolling norm
        done = False
        ep_reward = 0.0
        is_bankrupt = False
...
def validate(config_path, checkpoint_path, out_dir, episode_num, args):
    ...
    # 2. Load data
    # MODIFIED: Skip loading norm_stats.json for Rolling Z-Score mode.
    # We will generate dummy stats later to satisfy TradingEnvironment.
    logger.info("Rolling Z-Score mode: Skipping norm_stats.json loading. Using raw data.")
    norm_stats = {}
    ...
    # MODIFIED: Create dummy norm_stats (mean=0, std=1) for TradingEnvironment
    # This ensures that if the environment tries to denormalize, it gets the raw values back (since we passed raw values).
    num_channels = val_seqs[0].shape[0]
    for key in val_keys:
        asset_name = key.split('_')[0]
        if asset_name not in norm_stats:
            norm_stats[asset_name] = {'mean': [0.0]*num_channels, 'std': [1.0]*num_channels}

...
def run_validation():
    ...
    mask_close_action = backtest_cfg.get("mask_close_action", False)
    if mask_close_action:
        logging.info("⚠️  CLOSE action (3) is MASKED → TSL-only exits enabled")
    else:
        logging.info("✅ CLOSE action (3) is ENABLED → Agent controls exits")
    ...
    env_params = {
        ...
        "mask_close_action": mask_close_action,
        "cnn_format": False,
    ...
    for i in pbar:
        ...
        while not done:
            ...
            # Маскировка CLOSE действия для TSL-only режима
            if mask_close_action and action == 3:
                action = 0  # HOLD вместо CLOSE
...
def run_validation_with_config(config_path: str, model_path: str = None, override_params: dict = None) -> dict:
    ...
    if mask_close_action:
        logging.info("⚠️  CLOSE action (3) is MASKED → TSL-only exits")
    model_cfg = train_cfg_dict.get("model", {})
    rl_cfg = train_cfg_dict.get("rl", {})
    per_cfg = train_cfg_dict.get("per", {})
    eps_cfg = train_cfg_dict.get("eps", {})

...
cfg.model.cnn_maps = [64, 96, 128, 128, 96]  # Larger model, but optimized
...
cfg.trainlog.num_val_ep = 100  # Faster validation
cfg.trainlog.val_freq = 500  # Every 500 eps
...
cfg.trainlog.validation_warmup_steps = 10000  # Shorter warmup
# Увеличиваем общий горизонт обучения (качество > скорость)
cfg.trainlog.episodes = 30000  # Shorter horizon for faster convergence
cfg.trainlog.plot_top_n = 10
cfg.per.buffer_size = 500000
cfg.rl.batch_size = 128
# Стабильнее обновления с меньшим шагом
cfg.rl.learning_rate = 2e-5  # Faster learning
...
cfg.vec.num_envs = 1  # Stable on Windows
# CRITICAL: Subproc backend for Windows multiprocessing
cfg.vec.backend = "subproc"  # ✅ 10x speedup! Real parallel execution
...
cfg.random_seed = 404
cfg.paths.config_name = "alpha_conv_fast_seed_404"

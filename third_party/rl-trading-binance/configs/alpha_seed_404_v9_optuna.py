...
cfg.episodes_per_epoch = 3000  # Вместо 10000 - меньше данных загружать
...
cfg.rl.train_start = 3000  # Вместо 15000 - раньше начать обучение
cfg.rl.target_update_freq = 1000  # Вместо 2000 - чаще обновлять target
cfg.rl.max_gradient_norm = 1.0  # Clip grads

# DQN-specific (PER/epsilon)
cfg.per.buffer_size = 100000  # Вместо 1000000
cfg.per.per_alpha = 0.6
cfg.per.per_beta_start = 0.4
cfg.per.per_beta_frames = 45000  # Согласовано с total_timesteps
cfg.per.per_eps = 1e-6
cfg.eps.eps_start = 1.0
cfg.eps.eps_end = 0.05
cfg.eps.eps_decay_frames = 45000  # Согласовано с total_timesteps

# Env/Vectorized
cfg.vec.num_envs = 4  # Вместо 8 - меньше overhead на синхронизацию
...
cfg.trainlog.num_val_ep = 30  # Вместо 750 - быстрая валидация

# При 4 env один эпизод даёт ~4× больше шагов.
# Чтобы общий бюджет шагов остался ≈600k, эпизодов можно делать ~в 4 раза меньше.
cfg.trainlog.episodes = 186  # 3 валидации: на 62, 124, 186 эпизоде
cfg.trainlog.total_timesteps = 45000  # ~186 ep × 8 env × 30 steps

# Валидация: масштабируем по эпизодам, чтобы частота и прогрев соответствовали новому числу эпизодов.
cfg.trainlog.val_freq = 62  # Валидация каждые 62 эпизода = ровно 3 раза
cfg.trainlog.validation_warmup_steps = 0  # ОТКЛЮЧИТЬ warmup для быстрого старта
...
cfg.trainlog.early_stopping_patience = 999  # Отключить - пусть идет до конца
...
#   "min_win_rate": 0.42,
    "min_trades": 10,  # Ослабленный порог для промежуточных чекпоинтов
    "deny_inf_pf": True,
...
cfg.trainlog.save_top_k = 0  # Не сохранять промежуточные чекпоинты
...
cfg.backtest.mask_close_action = True  # Для оптимизации TSL - ОТКЛЮЧАЕМ CLOSE действие
...
# Random/Logging
cfg.random_seed = 404
cfg.paths.config_name = "alpha_seed_404_v9_optuna"  # Чтобы различать версии
...
#    "pos_frac":       ("suggest_float", 0.10,   0.60,   False, "market.position_fraction"),
    "d_min":          ("suggest_float", 0.001,  0.02,  True,  "backtest.trailing_stop_min"),
    "d0":             ("suggest_float", "d_min", 0.10,  True,  "backtest.trailing_stop"),
    "delta_p_hyst":   ("suggest_float", 0.0001, 0.01,  True,  "backtest.delta_p_hysteresis"),
...
cfg.paths.model_path = r"C:\Python\Prosperous_Bot\third_party\rl-trading-binance\output\alpha_seed_404_v9\saved_models\rl_binance_futures_trading_date_20251206_time_024756\best.pth"
cfg.paths.norm_stats_path = r"C:\Python\Prosperous_Bot\third_party\rl-trading-binance\output\alpha_seed_404_v9\saved_models\rl_binance_futures_trading_date_20251206_time_024756\norm_stats.json"

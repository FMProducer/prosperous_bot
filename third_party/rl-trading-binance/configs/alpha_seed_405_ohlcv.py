...
if AGENT_MODE == "UNIVERSAL":
    cfg.paths.config_name = "alpha_seed_405_ohlcv"
else:
    cfg.paths.config_name = f"alpha_seed_405_ohlcv_{AGENT_MODE}"
...
cfg.trainlog.episodes = 6000       # Оптимально для SHORT_ONLY
cfg.trainlog.total_timesteps = 350000  # Достаточный бюджет шагов

# Валидация: масштабируем по эпизодам, чтобы частота и прогрев соответствовали новому числу эпизодов.
cfg.trainlog.val_freq = 150             # Частая проверка для save_top_k
cfg.trainlog.validation_warmup_steps = 30000       # Даем модели время "осмотреться"
...
long_model_dir = BASE_DIR / "output" / "alpha_seed_405_ohlcv_LONG_ONLY" / "saved_models" / "rl_binance_futures_trading_date_20260110_time_133651"
short_model_dir = BASE_DIR / "output" / "alpha_seed_405_ohlcv_SHORT_ONLY" / "saved_models" / "rl_binance_futures_trading_date_20260110_time_191747"
single_model_dir = BASE_DIR / "output" / "alpha_seed_405_ohlcv_UNIVERSAL" / "saved_models" / "rl_binance_futures_trading_date_20251120_time_015257"

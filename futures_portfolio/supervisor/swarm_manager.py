...
from scanner.rank_tickers import main as run_scanner
from core.storage import safe_load_json, safe_save_json, safe_load_json_sync, safe_save_json_sync
from core.connector import BinanceConnector

# Настройка логирования
BASE_PATH = Path(__file__).resolve().parent.parent
CORE_PATH = BASE_PATH / "core"  # State-файлы хранятся в core/
...
async def get_pm2_processes() -> List[Dict[str, Any]]:
    ...
    """
    try:
        # create_subprocess_exec не находит pm2.cmd на Windows — нужен shell
        proc = await asyncio.create_subprocess_shell(
            "pm2 jlist --no-color",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
...
async def enforce_swarm_consistency(connector: BinanceConnector, config: dict) -> Set[str]:
    ...
    for pos_key in active_positions.keys():
        ...
        state_path = CORE_PATH / f"real_state_{ticker}.json"
    ...
    # Ensure subprocess can find core/ module from any CWD
    _sub_env = os.environ.copy()
    _sub_env["PYTHONPATH"] = str(BASE_PATH)

    for ticker in to_close_tickers:
        logger.info(f"🧹 Closing all positions for unauthorized ticker: {ticker}")
        cmd_stop = f'"{sys.executable}" "{BASE_PATH / "core" / "main.py"}" --config config.json --ticker {ticker} --stop --real'
        await (await asyncio.create_subprocess_shell(
            cmd_stop, env=_sub_env,
            stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL
        )).wait()
...
async def reset_bot_state_files(ticker: str, is_paper: bool, config: dict) -> None:
    ...
    base_state_path = CORE_PATH / base_state_file
    shadow_state_path = CORE_PATH / shadow_state_file
...
async def start_bot(ticker: str, is_paper: bool = True, config: dict = None):
    ...
    cmd = f'pm2 start core/main.py --name "{proc_name}" --cwd "{BASE_PATH}" --update-env --interpreter "{sys.executable}" --instances 1 -- --config config.json --ticker {ticker} {mode_flag}'
    await (await asyncio.create_subprocess_shell(
...
async def get_bot_efficiency(ticker: str, config: dict, is_real: bool = False) -> dict:
    ...
    """
    if is_real:
        state = await safe_load_json(str(CORE_PATH / f"real_state_{ticker}.json"), {})
    else:
        state = await safe_load_json(str(CORE_PATH / f"paper_state_{ticker}.json"), {})
    min_cycles = config.get("min_cycles_for_rank", 10)
...
async def manage_swarm():
    ...
    for mode, bl_dict, bl_key in [("paper", bl_paper, "toxic_blacklist_paper"), ("real", bl_real, "toxic_blacklist_real")]:
        ...
        for t, exp in bl_dict.items():
            if exp > now_ts:
                ...
                state_path = CORE_PATH / f"{mode}_state_{t}.json"
                if state_path.exists():
                    try:
    ...
    for key, info in list(running_bots.items()):
        ...
        state_path = CORE_PATH / f"{mode_prefix}_state_{ticker}.json"
        if state_path.exists():
            try:
    ...
    await (await asyncio.create_subprocess_shell(
        "pm2 save", stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL
    )).wait()

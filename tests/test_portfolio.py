...
async def test_get_value_distribution_usdt(mocker):
    ...
    values = await pm.get_value_distribution_usdt(p_spot=Decimal("50000"), p_contract=Decimal("250"), leverage=leverage)
...
async def test_get_value_distribution_with_leverage(mocker):
    ...
    mock_long_margin = Decimal("100.0")
    mock_short_margin = Decimal("50.0")
    ...
    test_cases = [
        ...
        (Decimal("5.0"), Decimal("50000"), Decimal("50000")), # Default leverage
        (Decimal("10.0"), Decimal("50000"), Decimal("50000")), # Custom leverage
        (Decimal("1.0"), Decimal("60000"), Decimal("60000")),  # Leverage of 1
        (Decimal("2.5"), Decimal("40000"), Decimal("40000")), # Fractional leverage
    ]
    ...
    for leverage, p_spot, p_contract in test_cases:
        ...
        if expected_total_value == 0:
            expected_distribution = {
                "BTC_SPOT": Decimal("0.0"),
                "BTC_PERP_LONG": Decimal("0.0"),
                "BTC_PERP_SHORT": Decimal("0.0"),
            }
        else:
            expected_distribution = {
                "BTC_SPOT": expected_spot_val / expected_total_value,
                "BTC_PERP_LONG": expected_long_val / expected_total_value,
                "BTC_PERP_SHORT": expected_short_val / expected_total_value,
            }

        # Test async version
        actual_values_async = await pm.get_value_distribution_usdt(p_spot=p_spot, p_contract=None, leverage=leverage) # Leverage parameter was missing in original call
        assert actual_values_async.keys() == expected_distribution.keys() # This was actual_values_usdt.keys()
        for key in expected_distribution:
            assert actual_values_async[key] == expected_distribution[key], \
                f"Async mismatch for {key} with leverage {leverage}, p_spot {p_spot}"

    # Test with no positions (only spot)
    futures_api_mock.futures.list_positions = mocker.Mock(return_value=[])
    leverage = Decimal("5.0")
    p_spot = Decimal("50000")
    p_contract = Decimal("50000")
    ...
    if expected_total_value_only == 0:
        expected_distribution_spot_only = {"BTC_SPOT": Decimal("0.0"), "BTC_PERP_LONG": Decimal("0.0"), "BTC_PERP_SHORT": Decimal("0.0")}
    else:
        expected_distribution_spot_only = {
            "BTC_SPOT": Decimal("1.0"),
            "BTC_PERP_LONG": Decimal("0.0"),
            "BTC_PERP_SHORT": Decimal("0.0"),
        }
    ...
    for key in expected_distribution_spot_only:
        assert actual_values_async_spot_only[key] == expected_distribution_spot_only[key], \
            f"Async mismatch for {key} (spot only)"

    # Test with no spot or positions (all zero)
    # Reconfigure mocks for the existing pm instance
    spot_api_mock.spot.get_account_detail = mocker.Mock(return_value=[
         gate_api.SpotAccount(currency="BTC", available="0.0")
    ])
    futures_api_mock.futures.list_positions = mocker.Mock(return_value=[]) # Futures are zero

    expected_distribution_all_zero = {"BTC_SPOT": Decimal("0.0"), "BTC_PERP_LONG": Decimal("0.0"), "BTC_PERP_SHORT": Decimal("0.0")}

    actual_values_async_all_zero = await pm.get_value_distribution_usdt(
        p_spot=p_spot, p_contract=p_contract, leverage=leverage
    )
...
def test_get_value_distribution_sync_with_leverage(mocker):
    ...
    test_cases = [
        (Decimal("5.0"), Decimal("50000"), Decimal("50000")),
        (Decimal("10.0"), Decimal("50000"), Decimal("50000")),
        (Decimal("1.0"), Decimal("60000"), Decimal("60000")),
        (Decimal("2.5"), Decimal("40000"), Decimal("40000")),
    ]
    ...
    for leverage, p_spot, p_contract in test_cases:
        ...
        actual_values_sync = pm.get_value_distribution_sync(p_spot=p_spot, p_contract=None, leverage=leverage)
        assert actual_values_sync.keys() == expected_distribution.keys()
        for key in expected_distribution:
            assert actual_values_sync[key] == expected_distribution[key], \
                f"Sync mismatch for {key} with leverage {leverage}, p_spot {p_spot}"

    # Test sync with no positions (only spot)
    futures_api_mock.futures.list_positions = mocker.Mock(return_value=[])
    leverage = Decimal("5.0")
    p_spot = Decimal("50000")
    p_contract = Decimal("50000") # Not used if no futures positions with margin
    expected_spot_val_only = mock_spot_btc_qty * p_spot
    expected_total_value_only = expected_spot_val_only
    ...
    for key in expected_distribution_spot_only:
        assert actual_values_sync_spot_only[key] == expected_distribution_spot_only[key], \
    ...
    futures_api_mock.futures.list_positions = mocker.Mock(return_value=[]) # No positions

    expected_distribution_all_zero = {"BTC_SPOT": Decimal("0.0"), "BTC_PERP_LONG": Decimal("0.0"), "BTC_PERP_SHORT": Decimal("0.0")}

    actual_values_sync_all_zero = pm.get_value_distribution_sync(
        p_spot=p_spot, p_contract=p_contract, leverage=leverage
    )

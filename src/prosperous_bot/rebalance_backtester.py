...
def run_backtest(params_dict, data_path, is_optimizer_call=True, trial_id_for_reports=None):
    ...
    if "date_range" in params and params["date_range"].get("start_date"):
        df_market = df_market[df_market['timestamp'] >= pd.to_datetime(params["date_range"]["start_date"])]
    if "date_range" in params and params["date_range"].get("end_date"):
        df_market = df_market[df_market['timestamp'] <= pd.to_datetime(params["date_range"]["end_date"])]
...
[end of src/prosperous_bot/rebalance_backtester.py]

...
def run_backtest(params_dict, data_path, is_optimizer_call=True, trial_id_for_reports=None):
    ...
    if orders_by_step: # Only run if there were orders to simulate
        ...
        if simulated_trade_log:
            ...
            # --- Ensure empty CSV is still written to avoid silent absence ---
            # Determine the path for the CSV file, same logic as above
            if generate_reports and output_dir:
                rebalance_trades_csv_path = os.path.join(output_dir, "rebalance_trades.csv")
            else:
                os.makedirs("reports", exist_ok=True) # Ensure 'reports' directory exists for fallback
                rebalance_trades_csv_path = "reports/rebalance_trades.csv"

            trades_sim_df = pd.DataFrame(columns=["asset_key", "entry_price", "exit_price", "qty", "pnl_gross_quote", "leverage"])
            trades_sim_df.to_csv(rebalance_trades_csv_path, index=False)
